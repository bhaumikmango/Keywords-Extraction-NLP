...
class KeywordExtractorModel(mlflow.pyfunc.PythonModel):
    """
    Wraps CountVectorizer + TfidfTransformer + feature_names into a
    single serialisable MLflow artefact.
    """
    ...
    def predict(self, context, model_input):
        """
        model_input: pandas DataFrame with columns ['text', 'top_n']
        Returns list of dicts {keyword: score}
        """
        import pandas as pd
        results = []
        for _, row in model_input.iterrows():
            ...
            kw     = {self.feature_names[i]: round(float(s), 4) for i, s in items
